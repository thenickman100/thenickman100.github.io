...
let _legendFracIsUserDrag = false;  // true only when _legendFrac was set by an actual user drag
...
function updateLegend() {
    ...
    let tx, ty;
    if (isMobileLand) {
        // Landscape: always pin legend to left edge, vertically centered.
        // Mobile layout modes override any saved fraction so resizing/rotating
        // always produces a sensible result without user intervention.
        tx = margin;
        ty = Math.round((H - legendH) / 2);
    } else if (isMobilePort && !_legendFracIsUserDrag) {
        // Portrait: always center legend horizontally at bottom, unless the
        // user has explicitly dragged it somewhere themselves this session.
        tx = Math.round((W - legendW2) / 2);
        ty = H - legendH - margin;
    } else if (_legendFrac !== null) {
        // User dragged the legend (or desktop restore): honour saved fraction.
...
function restoreExportedState(state) {
    ...
    // Restore saved legend position if we didn't refit.
    // Never restore on mobile portrait — the portrait branch in updateLegend
    // will center the legend correctly, and a desktop-derived fraction would
    // place it off the narrow screen.  _legendFracIsUserDrag is intentionally
    // NOT set here; only an actual user drag within this session sets it.
    ...
    } else if (didRefit || _restoreIsMobilePort) {
        _legendFrac = null;
    }
