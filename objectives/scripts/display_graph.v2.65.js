...
// Returns true when we're on a narrow touch device in landscape orientation.
// Used to switch the legend to the left and give the graph the right portion.
function _isMobileLandscape() {
    return (
        window.innerWidth > window.innerHeight &&  // landscape
        window.innerWidth < 1024 &&               // not a desktop/tablet
...
function updateLegend() {
    ...
    const isMobileLand = _isMobileLandscape();
    const isMobilePort = !isMobileLand && window.innerWidth < 1024 &&
                         ('ontouchstart' in window || navigator.maxTouchPoints > 0);
...
function restoreExportedState(state) {
    ...
    // Restore saved legend position only if we didn't refit
    // (refit sets its own _legendFrac above).
    if (!didRefit) {
